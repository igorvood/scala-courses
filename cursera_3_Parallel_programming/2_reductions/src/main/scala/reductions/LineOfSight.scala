...
object LineOfSight extends LineOfSightInterface :
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    val max: Float = (from until until)
      .map(idx =>
        if (idx == 0) 0
        else input(idx) / idx
      )
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
              threshold: Int): Tree = {
    val diapazon = end - from
    if (diapazon <= threshold) {
      Tree.Leaf(from, end, upsweepSequential(input, from, end))
    } else {
      val half = diapazon / 2
      val (left, right): (Tree, Tree) = parallel(upsweep(input, from, from + half, threshold), upsweep(input, from + half, end, threshold))
  ...
  def downsweepSequential(
    ...
    (from until until)
      .foreach(idx => output(idx) = max(input(idx) / idx, startingAngle))
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
                tree: Tree): Unit = {
    tree match {
      case Tree.Node(left, right) => parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, max(startingAngle, left.maxPrevious), right))
